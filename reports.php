...
$report = new DchSalesReport($db, $conf);
...
<form method="GET" action="<?php echo $_SERVER['PHP_SELF']; ?>" style="border:1px solid #ddd;border-radius:6px;padding:12px;margin-bottom:15px;display:flex;gap:12px;align-items:end;flex-wrap:wrap;">
  ...
  <label class="dch-month-field">Month<br><select class="flat" name="month"><?php for ($m = 1; $m <= 12; $m++) { ?><option value="<?php echo $m; ?>"<?php echo $filters['month'] === $m ? ' selected' : ''; ?>><?php echo date('F', mktime(0, 0, 0, $m, 1)); ?></option><?php } ?></select></label>
  <label class="dch-year-field">Year<br><input class="flat" type="number" min="2000" max="2100" name="year" value="<?php echo (int) $filters['year']; ?>" style="width:90px;"></label>
  <label class="dch-range-field">From<br><input class="flat" type="date" name="from" value="<?php echo dol_escape_htmltag($filters['from']); ?>"></label>
  <label class="dch-range-field">To<br><input class="flat" type="date" name="to" value="<?php echo dol_escape_htmltag($filters['to']); ?>"></label>
...
function dchReportPeriod() {
  var value = document.getElementById('dchPeriod').value;
  document.querySelectorAll('.dch-month-field').forEach(function(el) { el.style.display = value === 'month' ? '' : 'none'; });
  document.querySelectorAll('.dch-year-field').forEach(function(el) { el.style.display = value === 'month' || value === 'year' ? '' : 'none'; });
  document.querySelectorAll('.dch-range-field').forEach(function(el) { el.style.display = value === 'range' ? '' : 'none'; });
}
dchReportPeriod();
</script>
